...
def validate(text, output=None, software=False, limit=False):
    tag = "ROCK5_LIMIT" if limit else "ROCK5_PRESSURE"
    maximum = 1 if limit else 32
    mode = '--software' if software else '--native'
    assert text.count(tag+'_READY version=1 mode='+mode+
        ' chunk_size=262144 initial_chunks=1 max_chunks='+str(maximum)) == 1
    assert text.count(tag+'_PASS contexts=2 frames=4 pixels=25996 '
        'guard_bytes=512 software='+str(int(software))) == 1
    for forbidden in ('ROCK5_PIPELINE_FAILURE', 'ROCK5_PIPELINE_SHADER_FAILURE',
            tag+'_LINK_FAILURE', 'ROCK5_MESA_FAILURE', 'PANIC:',
            'Kernel Debugging Land', 'DEBUGGER:', 'mutex->owner'):
        assert forbidden not in text
    contexts = re.findall(r'^'+tag+r'_GL cycle=(\d+) renderer=(.*?) version=(.*?)\s*$', text, re.M)
    ...
    assert re.findall(r'^'+tag+r'_CONTEXT_PASS cycle=(\d+) rounds=2 destroyed=1\s*$',
        text, re.M) == ['0', '1']
    assert re.findall(r'^'+tag+r'_VERTICES cycle=(\d+) vertices=98304 bytes=786432 explicit_buffer=1\s*$', text, re.M) == ['0', '1']
    pattern = (r'^'+tag+r'_PIXELS_BEGIN cycle=(\d+) round=(\d+) quads=(\d+) '
        r'width=(\d+) height=(\d+) format=RGBA8 origin=lower-left\n(.*?)'
        r'^'+tag+r'_PIXELS_END cycle=(\d+) round=(\d+) mismatches=(\d+) '
        r'guard_errors=(\d+) wait=([0-9a-f]+) render_us=(\d+)\s*$')
    records = re.findall(pattern, text, re.M | re.S)
    assert len(records) == 4
    assert text.count(tag+'_PIXELS_BEGIN ') == 4
    assert text.count(tag+'_PIXELS_END ') == 4
    frames = []
    ...
    result = dict(status='pass', software=software, contexts=2, frames=frames,
        pixels=25996, guard_bytes=512, chunk_size=262144, initial_chunks=1, max_chunks=maximum, memory_limited=limit)
    if software:
        ...
        heaps = re.findall(r'^'+tag+r'_HEAP cycle=(\d+) phase=(\d+) clients=(\d+) '
        ...
        for index, values in enumerate(heaps):
            ...
            assert 1 <= chunks <= maximum and size == 4096 + chunks*262144
            assert chunks == 1 if limit or phase == 0 else chunks > 1
            if phase == 2:
