...
{
	assert(argc == 11);
	const unsigned segments[] = {0, 1, 2, 3, 4};
	const uint64_t roots[] = {0xa40000000ULL, 0xa40400000ULL,
		0xa40800000ULL, 0xa40c00000ULL, 0xa41000000ULL};
	const uint64_t endpoints[] = {0x900100000ULL, 0x940100000ULL,
		0x980100000ULL, 0x9c0100000ULL, 0xa00100000ULL};
	const uint64_t windows[] = {0xf0000000, 0xf1000000, 0xf2000000, 0xf3000000,
		0xf4000000};
	for (unsigned i = 0; i < 5; i++) {
		...
		memcpy(copy, endpoint, sizeof(copy));
		copy[1] &= ~2u;
		assert(EndpointMatches(copy, base, size, *port) == (i == 2));
		if (i != 0) {
			memcpy(copy, endpoint, sizeof(copy));
			copy[2] ^= 1;
			assert(!EndpointMatches(copy, base, size, *port));
		}
		const unsigned first = i < 3 ? 0 : 2;
		const unsigned second = i == 1 ? 5 : i == 2 ? 0 : 4;
		if (i == 2) {
			memcpy(copy, endpoint, sizeof(copy));
			copy[4] = uint32_t(base + size) | (copy[4] & 0xf);
			assert(!EndpointMatches(copy, base, size, *port));
			memcpy(copy, endpoint, sizeof(copy));
			copy[4] |= 1;
			assert(!EndpointMatches(copy, base, size, *port));
	...
	assert(!FindPort(0xa41400000ULL) && !FindPort(0xa40001000ULL));
	for (unsigned segment = 0; segment < 5; segment++) {
		MsiProfile profile;
		bool supported = FindMsiProfile(segment, profile);
		assert(supported == (segment == 0 || segment == 2));
		if (!supported)
			continue;
		uint32_t map[] = {profile.firmwareRequesterBase, 123,
			profile.firmwareRequesterBase, 0x1000};
		assert(MsiResourcesMatch(profile, map, sizeof(map), profile.controller, 0x20000));
		assert(profile.requesterBase == 0);
		for (unsigned cell : {0u, 2u, 3u}) {
			map[cell] ^= 1;
			assert(!MsiResourcesMatch(profile, map, sizeof(map), profile.controller, 0x20000));
			map[cell] ^= 1;
		}
		assert(!MsiResourcesMatch(profile, map, sizeof(map) - 1, profile.controller, 0x20000));
		assert(!MsiResourcesMatch(profile, map, sizeof(map), profile.controller + 0x20000,
			0x20000));
	}
	...
	puts("ROCK5_PCIE_ONBOARD_PROFILE_PASS functions=10 layouts=pass rejection=pass");
}
