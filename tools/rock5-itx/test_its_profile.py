"""Check ITS resource and requester identity admission against changed device trees."""
import os
...
class ItsProfileTests(unittest.TestCase):
    def test_fdt_and_controller_admission(self):
        ...
        with tempfile.TemporaryDirectory(dir=os.environ.get('TMPDIR')) as temporary:
            ...
            program = root / 'its-profile'
            subprocess.run(['g++', '-std=c++17', '-Wall', '-Wextra', '-Werror', *flags,
                str(directory / 'test_its_profile.cpp'), *objects, '-o', str(program)],
                check=True, capture_output=True, text=True)
            subprocess.run([str(program)], check=True, timeout=5)
            captured = os.environ.get('ROCK5_ITS_CAPTURED_DTB')
            if captured:
