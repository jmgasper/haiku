 ...
 */
#ifndef ARM64_RK3588_MBI_H
#define ARM64_RK3588_MBI_H
...
static const uint64_t kRedistributor = UINT64_C(0xfe680000);
...
{
	if (fdt == nullptr || fdt_check_header(fdt) != 0
...
RegistersMatch(uint32_t typer, uint32_t pidr2, uint32_t control, uint32_t group,
	uint32_t priority, uint32_t enabled, uint32_t pending, uint32_t active, uint32_t mask)
{
	...
	return typer == 0x7b040f && ((pidr2 >> 4) & 0xf) == 3
		...
		&& ((control & 0x40) != 0 ? (group & mask) != 0 : group == 0)
		&& ((enabled | pending | active) & mask) == 0;
}
