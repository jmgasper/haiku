...
Latest PCIe result: the `+156` USB candidate handled the captured SATA startup
condition on real hardware, waiting 2,587 us for Link Training to clear before
initializing all four ports. Both accepted boots and independent Linux eMMC
integrity passed. The SSD still runs `+148` pending this update;
[PCIE-TRAINING.md](PCIE-TRAINING.md) preserves the failures and tested scope.
