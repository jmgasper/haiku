...
| Onboard Ethernet | The +120 USB image passes DHCP and static IPv4/IPv6 at negotiated 2.5/1 Gbit/s, including IPv6 address replacement, discovery in both directions, route selection and concurrent traffic before/after reboot. The +156 SSD passes DHCP and four simultaneous IPv4 streams on each of two boots, with full physical-path captures and zero interface errors/drops. A +163 paired profiling run passed twelve 128 MiB + 7 byte streams with complete physical-path capture and zero errors/drops; full-stack sampling in a later trial exposed one continuously progressing but very slow TCP send and hit its deadline. Recovery and independent eMMC integrity passed. Throughput remains variable and below Linux; see [NETWORK-PROFILING.md](NETWORK-PROFILING.md). Automatic IPv6 configuration, forwarding, sustained load and fault recovery remain open. |
