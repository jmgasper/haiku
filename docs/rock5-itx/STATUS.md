...
| MMC / eMMC | The +144 USB image passes six eight-bit FAT file overwrites, explicit flushes, fresh mounts and persistence across normal reboot with CPU buffers forced above 4 GiB, with independent Linux file, filesystem and reference-region checks. The prior +136 four-bit image also passes orderly shutdown/startup. Cache remains disabled. Common SD/eMMC and FAT persistence pass QEMU. Eight-bit shutdown/startup, cached-card flush, power-loss integrity, faster clocks and eMMC boot remain pending. ROOBI is preserved; microSD uses a different host. See [MMC.md](MMC.md). |
