...
driver work is deferred at the owner's request. The +193 USB image passes all
138 host checks, the ARM64 build, both two-boot QEMU modes and two native boots.
Haiku now passes four CSF memory-store submissions using its own firmware and
application page tables, group/queue setup, completion objects and interrupts.
...
integrity pass. Shader execution and rendering remain pending.
...
CPU assertion remains recorded in [GPU.md](GPU.md). A checked shader reference
is next, followed by Haiku shader execution, rendering and Mesa integration.
