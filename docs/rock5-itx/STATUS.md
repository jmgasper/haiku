...
| MMC / eMMC | The +148 USB image passes eight-bit cached FAT file overwrites, explicit device-cache flushes, fresh mounts and persistence across normal reboot with CPU buffers forced above 4 GiB. Linux independently verifies files, the filesystem and reference regions. Earlier +144 also passes orderly shutdown/startup with cache disabled. Common SD/eMMC and FAT persistence pass QEMU. Cached shutdown/startup, power-loss integrity, sustained/error recovery, faster clocks and eMMC boot remain pending. ROOBI is preserved; microSD uses a different host. See [MMC.md](MMC.md). |
