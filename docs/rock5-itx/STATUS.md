...
| MMC / eMMC | The +144 USB image passes eight-bit FAT file overwrites, explicit flushes, fresh mounts and persistence across normal reboot and orderly shutdown/startup with CPU buffers forced above 4 GiB, with independent Linux file, filesystem and reference-region checks. Cache remains disabled. Common SD/eMMC and FAT persistence pass QEMU. Cached-card flush, power-loss integrity, sustained/error recovery, faster clocks and eMMC boot remain pending. ROOBI is preserved; microSD uses a different host. See [MMC.md](MMC.md). |
