...
driver work is deferred at the owner's request. The +190 USB image passes all
136 host checks, ARM64 build, both two-boot QEMU modes and two native boots.
Haiku now maps the Mali firmware, starts its MCU, validates interface 1.5.0 and
completes a doorbell ping. Each native boot records separate startup and ping
job interrupts on CPU 0, then stops the MCU, flushes GPU caches, removes the
address-space mapping and interrupt handlers, and restores all ten checked
platform registers. Both desktops, normal reboot/shutdown, automatic recovery
and independent recovery-image/eMMC integrity pass. The initial +189 attempt
