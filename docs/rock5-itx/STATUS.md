...
Latest installed result: the SSD runs `+156` and passes two boot, integrity,
...
| ARM64 system profiling | The +163 USB image passes 16 user/kernel/invalid-frame sampling checks across two native boots, plus 192 profiled fork/exec checks and 48 recovered faults, normal reboot, desktop and independent eMMC integrity checks. Corrections handle VHE EL2 frames, stale user frames during thread exit and an uninitialized empty user page table. Both QEMU modes and 113 host checks pass. Frame-pointer limitations, per-CPU placement and sustained overhead remain; see [PROFILING.md](PROFILING.md). The SSD remains +156. |
...
| Onboard Ethernet | The +120 USB image passes DHCP and static IPv4/IPv6 at negotiated 2.5/1 Gbit/s, including IPv6 address replacement, discovery in both directions, route selection and concurrent traffic before/after reboot. The +156 SSD passes DHCP and four simultaneous IPv4 streams on each of two boots, with full physical-path captures and zero interface errors/drops. Throughput remains variable and below Linux. Automatic IPv6 configuration, forwarding, sustained load and fault recovery remain open. |
