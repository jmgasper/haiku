...
| Work item | Starting point and dependencies | Acceptance |
| --- | --- | --- |
| Onboard eMMC | Verified eight-bit legacy SDR passes cached FAT file overwrites, explicit device-cache flushes, normal reboot and orderly shutdown/startup, including CPU buffers forced above 4 GiB. Linux independently verifies file/FS integrity and reference regions, and common SD/eMMC I/O passes QEMU. See [MMC.md](MMC.md) | Qualify power-loss integrity, concurrent/sustained I/O, error recovery, faster clock modes and Haiku boot; preserve the tested ROOBI recovery route |
